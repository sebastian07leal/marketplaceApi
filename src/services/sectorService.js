...
class SectorService {
    ...
    async createNewSector(data) {
        try {
            ...
            const response = await this._client.query(
                'INSERT INTO sector (id, name_sector, description) VALUES ($1, $2, $3);',
                [idSector, name, description],
            );
    ...
    async updateSectorById(id, data) {
        try {
            ...
            const response = await this._client.query(
                'UPDATE sector SET id = $1, name_sector = $2, description = $3 WHERE id = $4;',
                [idSector, name, description, id],
            );
            return response.rows[0];
        } catch (error) {
